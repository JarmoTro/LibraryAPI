...
<div class="col-7">
  ...
  <input type="date" name="loanStart" v-model="loanStart" class="form-control" placeholder="Loan Start">
        ...
        <p v-for="error of v$.$errors"
                :key="error.$uid">
                  <strong class="text-danger" v-if="error.$property == 'loanStart'">{{ error.$message }}</strong>
                </p>

    
            <div class="input-group m-3">
  <span class="input-group-text" id="basic-addon1">Loan End</span>
  <input type="date" name="loanEnd" v-model="loanEnd" class="form-control" placeholder="Loan End">
</div>

        <p v-for="error of v$.$errors"
                :key="error.$uid">
                  <strong class="text-danger" v-if="error.$property == 'loanEnd'">{{ error.$message }}</strong>
                </p>
